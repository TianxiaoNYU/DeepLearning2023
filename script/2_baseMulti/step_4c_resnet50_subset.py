...
train_data = TCGADataset_mo("./input/metadata/mo_train_dataset_subset.csv",train_transform)
valid_data = TCGADataset_mo("./input/metadata/mo_valid_dataset_subset.csv",validation_transform)
...
##n_class
n_class = pd.read_csv("./input/metadata/mo_train_dataset_subset.csv")['label']
...
for b in range(len(batch_size_set)):
  for l in range(len(learning_rate_set)):
    ...
    plt.savefig("./output/2_baseMulti/2_mo_resnet_performance_{b}_{l}.png".format(b = batch_size, l = learning_rate ))
    plt.clf()

df_train = pd.DataFrame( df_train, index = batch_size_set,columns = learning_rate_set)
df_train.to_csv("./output/2_baseMulti/2_mo_resnet_train_AUC.csv")
df_valid = pd.DataFrame( df_valid, index = batch_size_set,columns = learning_rate_set)
df_valid.to_csv("./output/2_baseMulti/2_mo_resnet_valid_AUC.csv")
