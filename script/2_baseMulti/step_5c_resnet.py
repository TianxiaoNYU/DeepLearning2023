...
n_epoch = 50
train_data = TCGADataset_mo("./input/metadata/mo_train_dataset.csv",train_transform)
valid_data = TCGADataset_mo("./input/metadata/mo_valid_dataset.csv",validation_transform)

##n_class
n_class = pd.read_csv("./input/metadata/mo_train_dataset.csv")['label']
...
for b in range(len(batch_size_set)):
  for l in range(len(learning_rate_set)):
    ...
    plt.savefig("./output/2_baseMulti/2_mo_whole_resnet_performance_{b}_{l}.png".format(b = batch_size, l = learning_rate ))
    plt.clf()


torch.save(model_trained, "./output/2_baseMulti/2_mo_resnetwhole_model.pt")
