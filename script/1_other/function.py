...
class TCGADataset_mo(Dataset):
    def __init__(self, csv_file, transform=None, 
        CNV_file = "/gpfs/home/chenz05/DL2023/input/metadata/CNV_top.csv",
        meth_file = "/gpfs/home/chenz05/DL2023/input/metadata/meth_top.csv",
        RNA_file = "/gpfs/home/chenz05/DL2023/input/metadata/RNA_top.csv",
        noise_fn = Gaussion_noise_fn):

        self.data_frame = pd.read_csv(csv_file)
        self.CNV_df = pd.read_csv(CNV_file)
        self.meth_df = pd.read_csv(meth_file)
        self.RNA_df = pd.read_csv(RNA_file)
...
def performance_fn(model,batch_size,learning_rate,train_data,valid_data,criterion,n_epoch = 50):
    ...
    for epoch in trange(1, n_epoch+1):
        ...
        if valid_loss <= valid_loss_best and valid_correct >= valid_correct_best and train_correct > 0.8 :  
            model_best = copy.deepcopy(model_trained)
            valid_loss_best = valid_loss * 1.1
