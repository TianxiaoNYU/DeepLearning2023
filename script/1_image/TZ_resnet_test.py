...
model_trained = torch.load("/gpfs/home/chenz05/DL2023/output/1_image/1_image_resnetwhole_model.pt")
model_trained.eval()
...
y_pred_1 = np.exp(y_pred)/sum(np.exp(y_pred))
cm = confusion_matrix(y_pred.argmax(1),y_true)
...
cm = confusion_matrix(y_pred_1.argmax(1),y_true)
print('Test confusion matrix:')
print(cm)
test_AUC = marco_AUC_performace(y_pred_1,y_true)[0]
print('Test AUC = {testAUC}'.format(testAUC = test_AUC))
