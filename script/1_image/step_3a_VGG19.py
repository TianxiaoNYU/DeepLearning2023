...
for b in range(len(batch_size_set)):
  for l in range(len(learning_rate_set)):
    ...
    model.classifier[-1]=torch.nn.Linear(in_features = 4096,out_features=n_class,bias=True)
    model = model.cuda()
    ...
    plt.savefig("./output/1_image/1_image_vgg_performance_{b}_{l}.png".format(b = batch_size, l = learning_rate ))
    plt.clf()

df_train = pd.DataFrame( df_train, index = batch_size_set,columns = learning_rate_set)
df_train.to_csv("./output/1_image/1_image_vgg_train_AUC.csv")
df_valid = pd.DataFrame( df_valid, index = batch_size_set,columns = learning_rate_set)
df_valid.to_csv("./output/1_image/1_image_vgg_valid_AUC.csv")
