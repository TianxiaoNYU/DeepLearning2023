...
learning_rate_set = [0.000001,0.00001,0.0001,0.001,0.005,0.01,0.1]
n_epoch = 100
...
for b in range(len(batch_size_set)):
  for l in range(len(learning_rate_set)):
    ...
    model.AuxLogits.fc = torch.nn.Linear(768, out_features=n_class,bias = True)
    model = model.cuda()
    batch_size = batch_size_set[b]
    learning_rate = learning_rate_set[l]
    train_dataloader = DataLoader(train_data ,batch_size=batch_size, shuffle=True)
    valid_dataloader = DataLoader(valid_data ,batch_size=batch_size, shuffle=True)
    model_trained,train_correct_set,valid_correct_set,train_loss_set,valid_loss_set = performance_fn_inception(model, batch_size,learning_rate,criterion = criterion, n_epoch  = n_epoch, train_data = train_data, valid_data  = valid_data )
    ...
    plt.savefig("./output/1_image/1_image_inception_performance_{b}_{l}.png".format(b = batch_size, l = learning_rate ))
    plt.clf()

df_train = pd.DataFrame( df_train, index = batch_size_set,columns = learning_rate_set)
df_train.to_csv("./output/1_image/1_image_inception_train_AUC.csv")
df_valid = pd.DataFrame( df_valid, index = batch_size_set,columns = learning_rate_set)
df_valid.to_csv("./output/1_image/1_image_inception_valid_AUC.csv")
