...
class multiomics_FC(torch.nn.Module):
    def __init__(self,dim_moinput = 5000, dim_mooutput = 2):
        super(multiomics_FC, self).__init__()
        self.model = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Dropout(0.2),
            torch.nn.Linear(in_features=128, out_features= dim_mooutput)
            )

    def forward(self,CNV,meth,RNA):
        out = self.model(torch.cat([CNV,meth,RNA],dim = 1))
        return out


def performance_fn2(model1, model2, batch_size,learning_rate,train_data,valid_data,criterion,rho,weight,n_epoch = 50):
    train_correct_set = []
    valid_correct_set = []
    train_loss_set = []
    valid_loss_set = []
    ...
    for epoch in trange(1, n_epoch+1):
        train_dataloader = DataLoader(train_data ,batch_size=batch_size, shuffle=True)
        valid_dataloader = DataLoader(valid_data ,batch_size=batch_size, shuffle=True)
        train_correct,train_loss, model1_trained, model2_trained = train_fn2(model1, model2, train_dataloader, criterion, rho, weight, optimizer)
        valid_correct,valid_loss,y_valid_pred,y_valid_true = valid_fn2(model1_trained, model2_trained, valid_dataloader, criterion, rho, weight, optimizer )
        ###loss & acc 
        train_correct = train_correct.cpu()
        valid_correct = valid_correct.cpu()
        train_correct = train_correct.double() / len(train_data)
        ...
        if valid_correct >= valid_correct_best:  
            model1_best = copy.deepcopy(model1_trained)
            model2_best = copy.deepcopy(model2_trained)
            #valid_loss_best = valid_loss
            valid_correct_best = valid_correct

    return model1_best, model2_best, train_correct_set,valid_correct_set,train_loss_set,valid_loss_set

def train_fn2(model1, model2, dataloader, criterion, rho,weight, optimizer):
    model1.train()
    model2.train()
    accum_train_loss = 0
    correct = 0
    ...
    for train_sample in dataloader:
        ...
        RNA = train_sample["RNA"].cuda()
        optimizer.zero_grad()
        y_pred1 = model1(image)
        y_pred2 = model2(CNV,meth,RNA)
        y_pred_total = y_pred1 + y_pred2
        loss = criterion(y_pred1, y_pred2, y, rho, weight)
        loss.backward()
        optimizer.step()
        accum_train_loss += loss.item()
        correct += sum(y == y_pred_total.argmax(1)).double()
    average_loss = accum_train_loss/len(dataloader)
    return correct,average_loss, model1, model2



def valid_fn2(model1, model2, dataloader, criterion, rho,weight, optimizer):
   with torch.no_grad():
     model1.eval()
     model2.eval()
     accum_valid_loss = 0
     ...
     for valid_sample in dataloader:
          ...
          correct += sum(y == y_pred_total.argmax(1)).double()
          y_pred_set.append(softmax(y_pred_total).cpu())
          y_true_set.append(y.cpu())
     average_loss = accum_valid_loss / len(dataloader)
     y_pred_set = [x.detach().numpy() for x in y_pred_set ]
     y_pred_set = numpy.concatenate( y_pred_set , axis=0 )
...
def predict_fn2(model1, model2, dataloader):
    model1.eval()
    model2.eval()
    y_pred = []
    y_true = []
    ...
    with torch.no_grad():
      for sample in dataloader:
        ...
        meth = sample["meth"].cuda()
        RNA = sample["RNA"].cuda()
        y_pred_total = model1(image) + model2(CNV,meth,RNA)

        y_pred.append(softmax(y_pred_total).cpu())
        y_true.append(y)
    y_pred = [x.detach().numpy() for x in y_pred ]
    y_pred = numpy.concatenate( y_pred , axis=0 )
    y_true = numpy.concatenate( y_true, axis=0 )
...
#class resnet50_co(torch.nn.Module):
#      def __init__(self, dim_image=2048,dim_CNV = 1000,dim_meth=3000,dim_RNA=1000):
#         super(resnet50_co, self).__init__()
#         resnet50_whole = models.resnet50(pretrained=True)
#         self.resnet50_avg = torch.nn.Sequential(*list(resnet50_whole.children())[:-1])
#         self.MLP_image = torch.nn.Sequential(
#                       torch.nn.Linear(in_features=dim_image, out_features= 512),
#                       torch.nn.ReLU(),
...
#                       )
#         self.MLP_CNV = torch.nn.Sequential(
#                       torch.nn.Linear(in_features=dim_CNV, out_features= 512),
#                       torch.nn.ReLU(),
#                       torch.nn.Dropout(0.3),
#                       torch.nn.Linear(in_features=512, out_features= 256),
#                       torch.nn.ReLU(),
...
#                       torch.nn.Linear(in_features=128, out_features= 1)
#                       )
#         self.MLP_meth = torch.nn.Sequential(
#                       torch.nn.Linear(in_features=dim_meth, out_features= 512),
#                       torch.nn.ReLU(),
#                       torch.nn.Dropout(0.3),
#                       torch.nn.Linear(in_features=512, out_features= 256),
#                       torch.nn.ReLU(),
...
#                       torch.nn.Linear(in_features=128, out_features= 1)
#                       )
#         self.MLP_RNA = torch.nn.Sequential(
#                      torch.nn.Linear(in_features=dim_RNA, out_features= 512),
#                      torch.nn.ReLU(),
#                       torch.nn.Dropout(0.3),
#                       torch.nn.Linear(in_features=512, out_features= 256),
#                       torch.nn.ReLU(),
...
#                       torch.nn.Linear(in_features=128, out_features= 1)
#                       )
#         self.CO_MLP = torch.nn.Linear(4,1,bias=True)
#    #         self.CO_MLP.weight = torch.nn.Parameter(torch.ones(1,4),requires_grad= False)
            
#      def forward(self,image,CNV,meth,RNA):
#         image_avg = self.resnet50_avg(image)
#         image_avg = torch.flatten(image_avg,1,3)
#         image_output = self.MLP_image(image_avg)
#         CNV_output = self.MLP_CNV(CNV)
#         meth_output = self.MLP_meth(meth)
#         RNA_output = self.MLP_RNA(RNA)
#         omics_output = {"image_output":image_output,
#                         "CNV_output":CNV_output,
#                         "meth_output":meth_output,
#                         "RNA_output:":RNA_output }
#         output = self.CO_MLP( torch.cat([image_output, CNV_output , meth_output , RNA_output],dim = 1))
#         y_pred = torch.sigmoid(output)

#         return y_pred, omics_output
...
class resnet50_co(torch.nn.Module):
      def __init__(self, dim_image=2048,dim_CNV = 1000,dim_meth=3000,dim_RNA=1000):
         ...
         #self.CO_MLP = torch.nn.Linear( 4 * 2 ,2,bias=True)
         #    self.CO_MLP.weight = torch.nn.Parameter(torch.ones(1,4),requires_grad= False)
         self.VM = vote_model(num_views = 4,bias=True)
      ...
      def forward(self,image,CNV,meth,RNA):
         ...
         omics_output = {"image_output":image_output,
                         ...
                         "RNA_output:":RNA_output }
         #output = self.CO_MLP( torch.cat([image_output, CNV_output , meth_output , RNA_output],dim = 1))
         output = self.VM(omics_output )
         y_pred = torch.nn.functional.softmax(output,dim = 1 )

         return y_pred, omics_output

class vote_model(torch.nn.Module):
    def __init__(self,num_views = 4,bias=True ):
        super(vote_model, self).__init__()
        self.weight = torch.nn.Parameter(torch.ones(num_views))
        if bias:
           self.bias = torch.nn.Parameter(torch.zeros(1))
    def forward(self, omics_output):
        outputs = 0
        for idx,omics in enumerate( omics_output):
            outputs += self.weight[idx] * omics_output[omics] + self.bias

        if omics_output[omics].shape[0] < 12:
           print("vote weight: ", self.weight, " vote bias: ",self.bias.item()  )
        return outputs



def regularization_fn(model, lamda1=0.001, lamda2=0.0):
    l1_loss = 0
    l2_loss = 0
    for param in model.parameters():
            l1_loss += torch.norm(param, p=1)
            l2_loss += torch.norm(param, p=2)
    return l1_loss * lamda1 + l2_loss * lamda2


#def multi_agreement_item_fn(rho,omics_output):
#      agreement_item = 0
#      n_sample = omics_output[ list(omics_output.keys())[0]].shape[0]
#      for i in range(1,len(omics_output)):
#       for j in range(i):
#          m1 = list(omics_output.keys() )[j]
#          m2 = list(omics_output.keys() )[i]
#          output1 = omics_output[m1]
#          output2 = omics_output[m2]
#          agreement_item += (output1 - output2)**2
#      return (sum(agreement_item) / n_sample) * rho/2

def selfdefine_cross_entropy(output1,output2):
    p = torch.nn.functional.softmax(output1,dim = 1)
    q = torch.nn.functional.softmax(output2,dim = 1)
    ce = -torch.sum(p * torch.log(q), dim=1)
    return ce.mean()
...
def multi_agreement_item_fn(rho,omics_output):
      ...
      for i in range(1,len(omics_output)):
       for j in range(i):
          ...
          output1 = omics_output[m1]
          output2 = omics_output[m2]
          agreement_item += selfdefine_cross_entropy(output1,output2)
          n_view_iter += 1
      return (agreement_item)/n_view_iter  * rho/2





#class CO_loss(torch.nn.Module):
#     def __init__(self, rho, lamda1,lamda2):
#         super(CO_loss, self).__init__()
#         self.rho = rho
#         self.lamda1 = lamda1
#         self.lamda2 = lamda2
#     
#     def forward(self, model, y_true, y_pred, omics_output ):
#         BCEloss = torch.nn.BCELoss()
#         nll_item = BCEloss( y_pred.squeeze(),y_true.float() )
#        # regular_item = regularization_fn(model,lamda1=self.lamda1, lamda2=self.lamda2) 
#         agreement_item =  multi_agreement_item_fn(self.rho,omics_output)
#        # co_loss = nll_item + regular_item + agreement_item
#         co_loss = nll_item  + agreement_item
#         return co_loss

class CO_loss(torch.nn.Module):
     def __init__(self,weight, rho, lamda1,lamda2):
         super(CO_loss, self).__init__()
         self.rho = rho
         self.lamda1 = lamda1
         self.lamda2 = lamda2
         self.weight = weight
     def forward(self, model, y_true, y_pred, omics_output ):
         nll_item = torch.nn.functional.nll_loss(torch.log(y_pred), y_true,weight = self.weight) 
         agreement_item =  multi_agreement_item_fn(self.rho,omics_output)
        # co_loss = nll_item + regular_item + agreement_item
         co_loss = nll_item  + agreement_item
         if y_pred.shape[0] < 12:
            print("CEL = {a}, agreement loss = {b}".format(a=nll_item, b=agreement_item))
         return co_loss



class CO_loss_test(torch.nn.Module):
    def __init__(self, rho, lamda1,lamda2):
        super(CO_loss_test, self).__init__()
        self.rho = rho
        self.lamda1 = lamda1
        self.lamda2 = lamda2
    ...
    def forward(self, model, y_true, y_pred, omics_output ):
        ...
        # co_loss = nll_item + regular_item + agreement_item
        co_loss = nll_item  + agreement_item
        print("BCE loss = {a}, agreement loss = {b}".format(a=nll_item, b=agreement_item))

        return co_loss         

def train_co_fn(model, dataloader,criterion,optimizer  ):
    model.train()
    accum_train_loss = 0
    correct = 0
    for train_sample in dataloader:
        ...
        RNA = train_sample["RNA"].cuda()
        optimizer.zero_grad()
        y_pred, omics_output = model(image,CNV,meth,RNA)
        loss = criterion(model,y,y_pred,omics_output )
        loss.backward()
        optimizer.step()
        accum_train_loss += loss.item()
        y_pred = y_pred[:,1]   ###ce needed  
        y_pred_label = torch.where(y_pred > 0.5, torch.tensor(1).cuda(), torch.tensor(0).cuda())
        correct += sum(y == y_pred_label.squeeze() ).double()
    average_loss = accum_train_loss/len(dataloader)
    return correct,average_loss, model



def valid_co_fn(model, dataloader, criterion,optimizer):
   with torch.no_grad():
     model.eval()
     accum_valid_loss = 0
     correct = 0
...
def predict_co_fn(model, dataloader):
    model.eval()
    y_pred = []
    y_true = []
    softmax = torch.nn.Softmax(dim = 1)
    with torch.no_grad():
      for sample in dataloader:
        ...
        meth = sample["meth"].cuda()
        RNA = sample["RNA"].cuda()
        y_pred_value, omics_output = model(image,CNV,meth,RNA)
        y_pred_value = y_pred_value[:,1]   ###ce needed 
        y_pred.append(y_pred_value.cpu())
        y_true.append(y)
    y_pred = [x.detach().numpy() for x in y_pred ]
    y_pred = numpy.concatenate( y_pred , axis=0 )
    y_true = numpy.concatenate( y_true, axis=0 )
...
def co_performance_fn(model,batch_size,learning_rate,train_data,valid_data,criterion,n_epoch = 50):
    train_correct_set = []
    valid_correct_set = []
    train_loss_set = []
    valid_loss_set = []
    ...
    for epoch in trange(1, n_epoch+1):
        train_dataloader = DataLoader(train_data ,batch_size=batch_size, shuffle=True)
        valid_dataloader = DataLoader(valid_data ,batch_size=batch_size, shuffle=True)
        train_correct,train_loss, model_trained = train_co_fn(model,train_dataloader, criterion, optimizer )
        valid_correct,valid_loss,y_valid_pred,y_valid_true = valid_co_fn(model_trained,valid_dataloader, criterion, optimizer )
        ###loss & acc 
        train_correct = train_correct.cpu()
        valid_correct = valid_correct.cpu()
        train_correct = train_correct.double() / len(train_data)
        ...
        y_valid_pred_label[y_valid_pred_label > 0.5 ] = 1
        y_valid_pred_label[y_valid_pred_label < 0.5 ] = 0
        cm = confusion_matrix(y_valid_pred_label,y_valid_true)
        print(cm)
        ###best model
        torch.save(model_trained, "./output/3_coLearn/0.9_3_co_resnetwhole_model_{epoch}_{learning_rate}.pt".format(epoch = epoch,learning_rate = learning_rate) ) 
        if valid_loss <= valid_loss_best and valid_correct >= valid_correct_best:
            model_best = copy.deepcopy(model_trained)
            valid_loss_best = valid_loss
            valid_correct_best = valid_correct
...
def co_marco_AUC_performace(y_pred, y_true ):
    fpr, tpr, _ = roc_curve(y_true , y_pred.squeeze())
    roc_auc = auc(fpr, tpr)
