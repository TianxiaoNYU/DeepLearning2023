...
for r in range(len(rho_list)):
  for l in range(len(lr_list)):
    ...
    rho = rho_list[r]
    lr = lr_list[l]
    criterion = CO_loss_test(rho = rho,lamda1 =0, lamda2 = 0)
    ...
    plt.savefig("/gpfs/home/chenz05/DL2023/output/3_coLearn/whole/whole_3_co_resnet_performance_{r}_{l}_pl.png".format(r =rho , l = lr))
    plt.clf()
    torch.save(model_trained, "/gpfs/home/chenz05/DL2023/output/3_coLearn/whole/coLearn_resnet50whole_model_{r}_{l}_pl.pt".format(r = rho, l = lr))

df_train = pd.DataFrame( df_train, index = rho_list,columns = lr_list)
df_train.to_csv("/gpfs/home/chenz05/DL2023/output/3_coLearn/whole/whole_3_co_resnet_train_AUC_0_pl.csv")
df_valid = pd.DataFrame( df_valid, index = rho_list,columns = lr_list)
df_valid.to_csv("/gpfs/home/chenz05/DL2023/output/3_coLearn/whole/whole_3_co_resnet_valid_AUC_0_pl.csv")
df_test = pd.DataFrame( df_test, index = rho_list,columns = lr_list)
df_test.to_csv("/gpfs/home/chenz05/DL2023/output/3_coLearn/whole/whole_3_co_resnet_test_AUC_0_pl.csv")
