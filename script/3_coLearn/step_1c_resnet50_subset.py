...
train_data = TCGADataset_mo("/gpfs/home/chenz05/DL2023/input/metadata/mo_train_dataset_subset.csv",train_transform)
valid_data = TCGADataset_mo("/gpfs/home/chenz05/DL2023/input/metadata/mo_valid_dataset_subset.csv",validation_transform)
import torchvision.models as models
df_train = np.zeros((len(rho_list),len(lr_list)))
df_valid = np.zeros((len(rho_list),len(lr_list)))

##n_class
n_class = pd.read_csv("/gpfs/home/chenz05/DL2023/input/metadata/mo_train_dataset_subset.csv")['label']
...
for r in range(len(rho_list)):
  for l in range(len(lr_list)):
    ...
    plt.savefig("/gpfs/home/chenz05/DL2023/output/3_coLearn/3_co_resnet_performance_{r}_{l}.png".format(r =rho , l = lr ))
    plt.clf()
